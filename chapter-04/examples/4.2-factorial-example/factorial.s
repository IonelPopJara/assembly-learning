...
factorial:
    ...
    movl 8(%ebp), %eax  #This moves the first argument to %eax
                        ...
                        #4(%ebp) holds the first parameter
    ...
    movl 8(%ebp), %ebx  #%eax has the reuturn value, so we
    ...
    imull %ebx, %eax    #multiply that by the result of the
                        ...
                        #is good since thatt's where return
                        #values go.
end_factorial:
    movl %ebp, %esp     #standard function return stuff - we
    popl %ebp           #have to resture %ebp and %esp to where
